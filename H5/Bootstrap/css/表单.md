...
| 验证类                 | 描述                         |
| ------------------- | -------------------------- |
| `.valid-feedback`   | 验证正确时输出，不带背景框              |
| `.valid-tooltip`    | 验证正确时输出，带背景框               |
| `.was-validated`    | 客户端验证，通常指浏览器前端页面验证         |
| `.invalid-feedback` | 验证错误时输出，不带背景框              |
| `.invalid-tooltip`  | 验证错误时输出，带背景框               |
| `.is-valid`         | 服务器端验证正确时输出，对应`.valid-*`   |
| `.is-invalid`       | 服务器端验证错误时输出，对应`.invalid-*` |
