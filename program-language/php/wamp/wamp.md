...
> -   我将安装包全部解压到了 `D:\server\wamp` 目录，子目录如下：
...
1.  apache2 主配置文件
    ...
    > 路径： `D:\server\wamp\apache2.4\conf\httpd.conf`

2.  apache2 虚拟主机配置文件

    > 路径： `D:\server\wamp\apache2.4\conf\extra\httpd-vhosts.conf`
    >
    > -   注意：需要在 httpd.conf 下开启相关配置

3.  php 主配置文件

    > -   php7.2 路径： `d:/server/wamp/php7.2/php.ini`
    > -   php7.1 路径： `d:/server/wamp/php7.1/php.ini`
    > -   php7.0 路径： `d:/server/wamp/php7.0/php.ini`
    > -   php5.6 路径： `d:/server/wamp/php5.6/php.ini`
    > -   注意：开发环境下拷贝 `php.ini-development` 为 \`php.ini
    > -   注意：生产环境下拷贝 `php.ini-production` 为 `php.ini`

4.  apache2 绑定 php 的模块所在位置

    > -   apache2 绑定 php7.2 ： `d:/server/wamp/php7.2/php7apache2_4.dll`
    > -   apache2 绑定 php7.1 ： `d:/server/wamp/php7.1/php7apache2_4.dll`
    > -   apache2 绑定 php7.0 ： `d:/server/wamp/php7.0/php7apache2_4.dll`
    > -   apache2 绑定 php5.6 ： `d:/server/wamp/php5.6/php5apache2_4.dll`
    > -   注意：只能一个版本生效，其他应该处于备注状态

5.  php 正常模块的默认路径
    > -   php7.2 正常模块路径： `d:/server/wamp/php7.2/ext` 下
    > -   php7.1 正常模块路径： `d:/server/wamp/php7.1/ext` 下
    > -   php7.0 正常模块路径： `d:/server/wamp/php7.0/ext` 下
    > -   php5.6 正常模块路径： `d:/server/wamp/php5.6/ext` 下
...
1.  为 apache2 配置正确的路径
    ...
    ```shell
    将 httpd.conf 文件下所有 `c:/Apache24` 替换成 `d:/server/wamp/apache2.4`
    ```
...
2.  为 apache2 配置当前服务器根目录
    ...
    ```conf
    DocumentRoot "d:/server/wamp/apache2.4/htdocs"
    <Directory "d:/server/wamp/apache2.4/htdocs">
    ```
...
3.  为 apache2 开启虚拟主机，并配置虚拟主机
    ...
    ```conf
    <VirtualHost *:80>
        DocumentRoot "d:/server/www/tp5/public"
...
6.  为 apache2 绑定 php
    ...
    LoadModule php7_module d:/server/wamp/php7.2/php7apache2_4.dll
    ## LoadModule php7_module d:/server/wamp/php7.1/php7apache2_4.dll
    ## LoadModule php7_module d:/server/wamp/php7.0/php7apache2_4.dll
    ## LoadModule php5_module d:/server/wamp/php5.6/php5apache2_4.dll
    ```
...
7.  让 `.htaccess` 文件支持伪静态
    ...
    > 接下来对指定的web根目录进行配置
    >
    > -   在 `<Directory "d:/server/www">` 内部修改，位置如下
...
> 启动 apache2 服务器（需要管理员权限的cmd）

```shell
net start apache2
```

> 关闭 apache2 服务器（需要管理员权限的cmd）
