...
- 在任何上下文之外放置在配置文件中的指令都被认为是在 main 上下文中；
    - 如：events 和 http 指令驻留在 main 上下文中， server 位于 http 中， location 位于 server 中。
```
...
> 关于 Nginx 配置文件的具体配置请查看 [Nginx 配置案例](./02.1-Nginx配置案例.md)
